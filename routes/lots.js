...
// GET /api/lots - Tutti i lotti
router.get('/', async (req, res) => {
    try {
        const rows = await db.allAsync('SELECT * FROM lots ORDER BY created_at DESC');
        res.json({
            message: 'success',
            data: rows,
            count: rows.length
...
router.post('/', async (req, res) => {
    try {
        const { company_name, location, gps_coordinates, product_type, product_category, variety, field_lot, field_size } = req.body;
        ...
        const result = await db.runAsync(
            `INSERT INTO lots (company_name, location, gps_coordinates, product_type, product_category, variety, field_lot, field_size) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
            [company_name, location, gps_coordinates, product_type, product_category || null, variety || null, field_lot || null, field_size || null]
        );
        
        const newLot = await db.getAsync('SELECT * FROM lots WHERE id = ?', [result.id]);
        
        console.log('✅ Lotto creato nel database:', {
            id: newLot.id,
            company_name: newLot.company_name,
            product_type: newLot.product_type,
            product_category: newLot.product_category, // DEBUG
...
router.post('/:id/details', async (req, res) => {
    try {
        ...
        const result = await db.runAsync(
            `INSERT INTO lot_details (lot_id, cost_per_kg, estimated_kg, purchase_date, harvested_kg) 
             VALUES (?, ?, ?, ?, ?)`,
            [req.params.id, cost_per_kg, estimated_kg, purchase_date, harvested_kg]
        );
